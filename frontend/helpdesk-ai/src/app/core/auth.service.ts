...
export class AuthService {
  ...
  private readonly tokenState = signal<string | null>(sessionStorage.getItem('helpdesk_token'));
  ...
  logout(): void { sessionStorage.removeItem('helpdesk_token'); sessionStorage.removeItem('helpdesk_user'); this.tokenState.set(null); this.userState.set(null); }
  private save(result: AuthResponse): void { sessionStorage.setItem('helpdesk_token', result.token); sessionStorage.setItem('helpdesk_user', JSON.stringify(result.user)); this.tokenState.set(result.token); this.userState.set(result.user); }
